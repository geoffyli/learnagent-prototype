...
const kindMeta: Record<SessionKind, { label: string; icon: JSX.Element; tone: string }> = {
  main: {
    label: 'Main',
...
export default function SessionCanvas({
  ...
  const branchNodes = useMemo(() => {
    ...
    return nodes.filter((n) => n.skillNodeId === effectiveSelectedId && n.kind !== 'topic');
    ...
    <motion.div className="flex h-full min-w-0 flex-col overflow-hidden" initial={false}>
      ...
      {mainPhase === 'planning' ? (
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          <motion.div
            className="flex w-44 shrink-0 flex-col overflow-y-auto border-r border-slate-200/70 bg-white/40 py-3"
