  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          key={i}
          className={`${cls} ${i < Math.round(score) ? 'fill-amber-400 text-amber-400' : 'fill-slate-200 text-slate-200'}`}
        />
      ))}
    </div>
  );
...
export default function CourseDetailPage({ pkg, community, onStartCourse, onBack }: CourseDetailPageProps) {
  ...
  return (
    ...
    >
      {/* Top nav */}
      <motion.header
        className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur-sm"
        variants={fadeSlideY(rm, 8, MOTION_DURATION.slow)}
      >
        <div className="mx-auto flex max-w-2xl items-center justify-between">
          <p className="font-heading text-xl font-semibold tracking-tight text-slate-900">LearnAgent</p>
          <motion.button
            type="button"
            onClick={onBack}
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-600 transition hover:border-teal-300 hover:text-teal-700"
          >
      ...
      {/* Hero banner */}
      <motion.div
        className="bg-gradient-to-br from-teal-600 to-teal-700 px-4 py-10"
        variants={fadeSlideY(rm, 12, MOTION_DURATION.slow)}
      >
        <div className="mx-auto max-w-2xl">
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-teal-200">Course</p>
          <h1 className="text-3xl font-bold text-white">{pkg.title}</h1>
          <p className="mt-2 text-base text-teal-100">{pkg.subtitle}</p>

          {/* Stats row */}
          <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-teal-100">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-sm text-teal-200">({community.rating.count.toLocaleString()} reviews)</span>
          </div>
        </div>
      </motion.div>
      ...
      <main className="mx-auto max-w-2xl space-y-4 px-4 py-6 pb-12">
        ...
        >
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-700">Ready to start?</p>
              <p className="mt-0.5 text-xs text-slate-400">{pkg.skillNodes.length} skill nodes · ~{totalMinutes} min total</p>
            </div>
            <motion.button
              type="button"
              ...
              whileTap={rm ? undefined : { scale: 0.98 }}
              transition={springFor(rm, 'snappy')}
              className="shrink-0 rounded-xl bg-teal-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-500"
            >
              Start Course
            </motion.button>
          </div>
          ...
          <div className="divide-y divide-slate-100">
            {pkg.skillNodes.map((node, i) => (
              <div key={node.id} className="flex items-start gap-4 px-5 py-3.5">
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-teal-50 text-xs font-bold text-teal-600">
                  {i + 1}
                </span>
                ...
                {node.estimatedMinutes != null && (
                  <span className="shrink-0 text-xs text-slate-400">{node.estimatedMinutes} min</span>
                )}
              </div>
            ))}
          </div>
        ...
        >
          <div className="border-b border-slate-100 px-5 py-4">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">About the Creator</h2>
          </div>
          <div className="flex items-start gap-4 px-5 py-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-slate-900">{community.creator.name}</p>
              <p className="mt-1.5 text-sm leading-relaxed text-slate-600">{community.creator.bio}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-slate-700">{community.rating.score}</span>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400">{comment.daysAgo}</span>
                    </div>
                    <StarRow score={5} size="sm" />
                  </div>
                </div>
                <p className="mt-2.5 text-sm leading-relaxed text-slate-600">{comment.text}</p>
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
        ...
        >
          <motion.button
            ...
            whileTap={rm ? undefined : { scale: 0.98 }}
            transition={springFor(rm, 'snappy')}
            className="w-full rounded-2xl bg-teal-600 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-500"
          >
            Start Course
          </motion.button>
        </motion.div>
